...
def handle_handoff(wrapper: RunContextWrapper, input_data:HandoffData):
    with st.sidebar:
        st.write(f"""
            Handing off to: {input_data.to_agent_name}
            Reason: {input_data.reason}
            Issus_type: {input_data.issue_type}
...
triage_agent = Agent(
    ...
    # tools=[  #다른 agent를 tool처럼 사용할 수 있음. 대화를 항상 main agent가 진행함.
